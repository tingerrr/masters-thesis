...
#pdfpc.speaker-note(
  ...
  - during measurement -> repeated execution in a certian time
  - t4gl programs -> microsteps -> time slices
  - generations are usually 1ms long
  - each t4gl green thread must be able to execute its steps
  - major problem
    - fixed len generations
    - var len data
  ```
)
...
#pdfpc.speaker-note(
  ```md
  - not writing directly -> easy sharing of common data
  - small differences -> lage commonality
  - new instances are copies of old instances with the new change
    (removal/insertion/etc.)
  - QMaps are CoW, a core building block of persistence
  ```
)
...
#pdfpc.speaker-note(
  ...
  - no improvement in the order of time complexity, chunking would simply change
    the constant factor
  - recursive chunking of chunks yields better results
  ```
)
...
#slide(repeat: 4, self => [
  #pdfpc.speaker-note(
    ```md
    - more green than red notes -> more sharing than copying
    - good balancing is key
    - segway to trees
    ```
  )
...
#pdfpc.speaker-note(
  ```md
  - 3 types of trees for various use cases
  - need not be persistent, but are for our use case
  ```
)
...
#pdfpc.speaker-note(
  ...
  - doesn't come with insert or remove by default, these require the key monoid
  - optimizaiton over b-tree on average, but not worst case
  ```
)
...
#pdfpc.speaker-note(
  ```md
  - as mentioned before, bounds must not degenerate
  - otherwise they have no improvement over b-trees
  ```
)
...
#slide(repeat: 8, self => [
  #pdfpc.speaker-note(
    ...
    - "safety" of nodes represents whether they can hold the amortized push and
      pop bounds
...
#pdfpc.speaker-note(
  ```md
  - logarithmic growth for all throughout
  - comparison of worst case trees to best case qmap
  - finger tree is compound of worst case split worst case push and worst case
    concat
