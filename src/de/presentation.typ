...
- jeder Schreibzugriff erzeugt eine neue veränderte Instanz
  #pause
- QMaps sind daher bis zu einem gewissen Grad persistent
...
#pdfpc.speaker-note(
  ```md
  - radix trees/prefix tries are trees which store items with common prefixes as children to a node with that prefix as key
  - example: "to", "todo", "rodeo"
  - simpler comparison per node
  ```
)

// TODO: include image of rrb-tree

#pause

- _engl._ relaxed radix balanced tree
  #pause
- Spezialisierung von Radix-Bäumen
  #pause
- Sequenzdatenstruktur, funktioniert nur mit Indextypen
  #pause
- `search`, `insert`, `remove`: $Theta(log n)$
...
- nicht zwangsläufig persistent
  #pause
- Komplexität für persistente Variante baut stark auf lazy evaluation auf
  #pause
...
Welche Art von Schlüsseln haben wir?

#pause

- totale Ordnungsrelation
  #pause
- nur zur Laufzeit bekannte Länge

#pause

Welche Operationen müssen generalisiert werden?
...
Welche Zweigfaktoren definieren wir?

#pause

- Digits: $d_min = 1, d_max = 4$
  #pause
- Knoten: $k_min = 2, k_max = 3$
...
#pdfpc.speaker-note(
  ```md
  - finger trees my be possible, the initial implementation was wholy unoptimized
  ...
  - real time analysis doen't care about average bounds
  ```
)
