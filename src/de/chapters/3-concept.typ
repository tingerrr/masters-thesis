...
Die T4gl-Instanz, welche an den Programmierer übergeben wird, sowie die interne Storage-Instanz, teilen sich einen Buffer, welcher vom Laufzeitsystem zwangsläufig beim nächsten Schreibzugriff kopiert werden muss.
...
+ T4gl-Arrays sind assoziative Arrays.
...
+ T4gl-Arrays sind in ihren Schlüsseln _nicht_ dicht verteilt.
  - Schlüssel wie `[0, 4, 6]` müssen verwaltet werden können ohne Zwischenschlüssel wie `[1, 2, 3, 5]` anlegen zu müssen.
+ T4gl-Arrays verhalten sich wie Referenztypen auf T4gl-Ebene.
...
Essentiell für die Verbesserung des _worst-case_ Zeitverhalten bei Kopien und Schreibzugriffen ist die Reduzierung der Daten, welche bei Schreibzugriffen kopiert werden müssen.
Hauptproblem bei Sequenzen von seichten Kopien gefolgt von Schreibzugriff auf CoW-Datenstrukturen ist die tiefe Kopie _aller_ Daten im Buffer der Instanzen, selbst wenn nur ein einziges Element beschrieben oder eingefügt/entfernt wird.
Ein Großteil der Daten im alten und neuen Buffer sind nach dem Schreibzugriff gleich.
Deshalb wurde als Basis des Lösungsansatz die Wahl einer persistenten Datenstruktur mit granularer Bufferteilung festgelegt.
Durch höhere Granularität der Bufferteilung müssen bei Schreibzugriffen weniger Daten kopiert werden.
Ein Beispiel für persistente Datenstrukturen mit granularer Bufferteilung sind RRB-Vektoren @bib:br-11 @bib:brsu-15 @bib:stu-15, eine Sequenzdatenstruktur auf Basis von Radix-Balancierten Bäumen.
...
Da die Schlüsselverteilungen in T4gl-Arrays nicht dicht ist, können die Schlüssel nicht ohne weiteres auf dicht verteilte Indizes abgebildet werden, welche für die Radixsuche essentiel sind.
...
Diese bieten sowohl exzellentes Zeitverhalten, sowie keine Enschränkung auf die Schlüsselverteilung.
...
#todo[
  The above could go with some examples, especially regarding RRB-Vectors and how they could be used as paired sequences ordered by key on insertion given that it's insert/remove bounds are not sublinear.
]
...
/ Digit: Erweiterung on 2-3-Knoten auf 1-4-Knoten von welchen jeweils zwei in einem internen Wirbelknoten vorzufinden sind.
...
/ Unsafe: Unsichere _Digits_ sind _Digits_ mit 1 oder 4 Elementen, ein Element zu entnehmen oder hinzuzufügen kann Über- oder Unterfluss verursachen.
...
Dabei ist zu sehen, dass _Push_ und _Pop_ Operationen auf 2-3-Fingerbäumen im _average-case_ amortisiert konstantes Zetiverhalten aufweisen, im Vergleich zu dem generell logarithmischen Zeitverhalten bei gewöhnlichen 2-3-Bäumen.
...
#todo[
  ...
  My assumption was that the rebalancing simply happens less and less often as it always creates safe layers when it happens, and the deeper it gets the more safe layers must unsafed before requireing rebalancing again.
]
...
= Generalisierung
Im Folgenden wird betrachtet inwiefern die Zweigfaktoren von Fingerbäumen generalisierbar sind ohne die in @tbl:finger-tree-complex beschribenen Komplexitäten zu verschlechtern.
...
Wir beschreiben die Struktur eines Fingerbaums durch die Minima und Maxima dessen Zweigfaktoren $k$ (Zweigfaktor der internen Knoten) und $d$ (Zweigfaktor der _Digits_)
...
Die Teilbäume, welche in den _Digits_ liegen, werden auf B-Bäume generalisiert, daher gilt für deren interne Knoten außer dem
...
Da jeder tiefe Wirbelknoten mindestens $2 d_min$ enthalten muss, muss die Bildung dieser minimal Anzahl der Elemente im letzten Wirbelknoten überbrückt werden.
...
Für die kummulativen Minima und Maxima aller Ebenen bis zur Ebene $t$ ergibt sich
...
Dabei zeigen die horizontalen Linien das kummulative Sonderminimum $n'_"smin"$ und Maximum $n'_max$ pro Ebene.
...
Eine Ebene $n$ mit $d$ Elementen gilt als sicher @bib:hp-06[S. 7], wenn
...
Ist eine Ebene unsicher, kommt es beim Hinzufügen oder Wegnehmen zum Über- oder Unterfluss, Elemente müssen zwischen Ebenen gewechselt werden um die obengenannten Ungleichungen einzuhalten.
...
Das ist möglich, solange eine unsichere Ebene nach Über- oder Unterfluss wieder sicher ist.
Dazu betrachten wir den Über- und Unterfluss einer unsicheren Ebene $t$ in eine oder aus einer sicheren Ebene $n + 1$.
Über- oder Unterfluss von unsicheren Ebenen in bzw. aus unsicheren Ebenen kann rekursiv angewendet werden bis eine sichere Ebene erreicht wird.

Der Wechsel von Elementen zwischen zwei Ebenen erfordert entweder
- das Anheben von Elementen der Ebene $t$ in Knoten zur Ebene $t + 1$ bei Überfluss,
- oder das Herunterlassen von Knoten der Ebene $t + 1$ zu Ebene $t$ bei Unterfluss.
...
Überfluss entsteht bei Hinzufügen eines Elements in eine Ebene mit $d_max$ Elementen, es werden Elemente aus Ebene $t$ zu Ebene $t + 1$ bewegt, die folgenden Ungleichungen müssen Eingehalten werden damit die Ebene $t$ nach dem Überfluss sicher ist.
...
Gleichermaßen gelten für den Unterfluss folgende Ungleichungen.
$
...
#todo[
  This is weirdly written and it feels out of place.
  It should also be noted that the choice of $dd1$ makes sense for overflow, whereas for underflow it is given by the amount stored in the unwrapped nodes from layer $t + 1$, so for underflow the choice of $dd2$ matters.
  The combinations of these choices may have ot be agreeable for both cases or are completely independent, I don't know yet.
  In case of independence, we need to call disambiguate these choices for each of them, chosing one so the other can be represented while maintaining the inequalities.
]

Wie in @bib:hp-06 beschrieben entschieden sich die Authoren für $3$, gaben aber an das $2$ ebenfalls Funktioniert.
...
Daraus ergibt sich das $d_min = 1$, $d_max = 5$, $k_min = 2$ und $k_max = 4$ einen validen 2-3-4-Fingerbaum beschreiben.
...
== Insert & Remove
#todo[
  Insert and remove in @bib:hp-06 are done using a split + push/pop + concat, I'm unsure whether this is a feasible thing to do when we need to carfeully manage the lazy evaluation ourselves.
...
= Echtzeitanalyse
#todo[
  Analyse the theoretical impact the new data structure _should_ have on the current runtime given its _worst-case_ bounds.
